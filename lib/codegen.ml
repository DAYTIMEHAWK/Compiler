...
type context = {
    ...
    free_temp_regs: string list;   (* 空闲临时寄存器列表 *)
    all_temp_regs: string list;    (* 所有可用临时寄存器 *)
...
let create_context func_name =
    ...
    { current_func = func_name;
      ...
      reg_map = reg_map;
      param_count = 0;
      free_temp_regs = all_temp_regs;
      all_temp_regs = all_temp_regs;
...
let alloc_temp_reg ctx =
    match ctx.free_temp_regs with
    | reg::rest -> 
        { ctx with free_temp_regs = rest }, reg
    | [] -> failwith "No more temporary registers available"

(* 释放临时寄存器 *)
let free_temp_reg ctx reg =
    { ctx with free_temp_regs = reg :: ctx.free_temp_regs }
...
let rec gen_expr ctx expr =
    ...
    | BinOp (e1, op, e2) ->
        ...
        in
        (* 释放临时寄存器 *)
        let ctx = free_temp_reg (free_temp_reg ctx reg2) reg1 in
        (ctx, asm1 ^ "\n" ^ asm2 ^ "\n" ^ instr, reg_dest)
    | UnOp (op, e) ->
...
| FuncCall (name, args) ->
      ...
      (* 总临时空间 = 保存所有临时寄存器(15*4=60) + 返回值保存(4) + 额外参数空间 *)
      let temp_space = 60 + 4 + n_extra * 4 in
      ...
      let move_args_asm = 
        let rec move_args regs index asm =
          ...
          | reg::rest ->
              let stack_offset = 64 + (index - 8) * 4 in  (* 60(寄存器保存区)+4(返回值保存) *)
      ...
      (* 将返回值移动到目标寄存器 *)
      let (ctx, reg_dest) = alloc_temp_reg ctx in
      let move_result = Printf.sprintf "    lw %s, 60(sp)" reg_dest in
      
      (* 组合汇编代码 *)
      let asm = arg_asm ^ stack_adj_asm ^ save_temps_asm ^ "\n" ^ 
                move_args_asm ^ call_asm ^ save_ret_asm ^ 
                restore_temps_asm ^ "\n" ^ restore_stack_asm ^ "\n" ^ 
                move_result in
        ...
        let rec gen_save params index asm =
            ...
            | param::rest ->
                ...
                if index < 8 then (
                    ...
                    let (_ctx_temp, reg) = alloc_temp_reg ctx in  (* 使用临时寄存器 *)
                    let load_asm = Printf.sprintf "  \n  lw %s, %d(sp)\n" reg (ctx.frame_size + stack_offset) in
